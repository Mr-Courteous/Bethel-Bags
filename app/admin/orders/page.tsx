    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {["PENDING","PAID","PROCESSING","SHIPPED","DELIVERED"].map((s) => {
          ...
          return (
            <div key={s} className={`p-4 border text-center ${STATUS_COLORS[s] || "bg-gray-50 text-gray-600"} border-current/20`}>
              <p className="font-serif text-2xl font-bold">{count}</p>
              <p className="text-xs uppercase tracking-widest mt-1 opacity-70">{s}</p>
            </div>
          );
        })}
      </div>
          ...
          <thead>
            <tr className="border-b border-gray-100">
              {["Order #", "Customer", "Items", "Total", "Status", "Date", ""].map((h) => (
                <th key={h + Math.random()} className="text-left px-5 py-3.5 text-xs tracking-widest uppercase text-empire-grey font-medium whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <Link href={`/admin/orders/${o.id}`} className="text-xs text-blue-600 hover:underline font-medium">View →</Link>
                </td>
              </tr>
            ))}
          </tbody>
